...
exports.home = function(req,res,next){
	...
	//首页第一次展示8条记录
	db("select id,title,brief,like_num,comment_num,tags from article order by create_time DESC limit 0,8",null,function(err,vals,fields){
		console.log(err);
...
exports.article = function(req,res,next){
	db("select * from article where id = ? limit 0,1",req.params.id,function(err,vals,fields){
		res.render('article',{articles : vals});
	})
};
