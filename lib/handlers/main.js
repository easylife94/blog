...
exports.home = function(req,res,next){
	...
	//首页第一次展示8条记录
	db("select id,front_url,title,brief,like_num,comment_num,tags,date_format(create_time, '%Y-%m-%d %H:%m:%S') create_time  from article order by create_time DESC limit 0,8",null,function(err,vals,fields){		
		console.log(err);
		if(vals != null){
...
exports.article = function(req,res,next){
	db("select * from article where id = ? limit 0,1",req.params.id,function(err,vals,fields){
		if(vals != null && vals.length > 0){
			//时间格式化
			vals[0].create_time = moment(vals[0].create_time).format("YYYY年M月d日 HH:mm");
			res.render('article',{article : vals[0]});
		}else{
			res.redirect('/article-not-found');
		}
...
exports.articleImg = function(req,res,next){
	...
	var path = config.blog.uploadImgDir+"/"+filename;
	console.log(path);
